...
async function loadAllSessions(): Promise<SessionInfo[]> {
  const [scanned, nestedSessions] = await Promise.all([
  ...
  const sessions: SessionInfo[] = scanned.map((s) => {
  ...
  // Nested subagent sessions (pi TUI layout) live outside the incremental
  // scan; merge them unless the scan already surfaced the same file/id.
