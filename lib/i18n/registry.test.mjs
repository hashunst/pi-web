...
test("uses the first supported browser language and falls back to English", () => {
  ...
  assert.equal(resolveBrowserLocale(["fa", "en-US"]), "fa");
  assert.equal(resolveBrowserLocale(["fa-IR", "fr-FR"]), "fa");
  assert.equal(resolveBrowserLocale(["FA-IR", "en-US"]), "fa");
  assert.equal(resolveBrowserLocale(["fr-FR"]), "en");
  assert.equal(resolveBrowserLocale([]), "en");
});

test("returns only registered locales", () => {
  assert.deepEqual(getSupportedLocales(), ["en", "zh-CN", "zh-TW", "fa"]);
  ...
  assert.equal(getLocalePlugin("fa")?.label, "فارسی");
  assert.equal(getLocalePlugin("fa")?.messages["common.language"], "زبان");
