...
function writeNestedSubagentLayout(agentDir, { parentId, childId, name, lastRole }) {
  const projectDir = join(agentDir, "sessions", "--project--");
  const parentStem = `2026-01-01T00-00-00-000Z_${parentId}`;
  const parentPath = join(projectDir, `${parentStem}.jsonl`);
  const runDir = join(projectDir, parentStem, childId, "run-0");
  const childPath = join(runDir, "session.jsonl");
  ...
  writeFileSync(parentPath, `${JSON.stringify({
    ...
    timestamp: "2026-01-01T00:00:00.000Z",
    cwd: "/tmp/project",
  })}\n${JSON.stringify(userEntry("pu1", null, "parent task"))}\n`);
  const lines = [
    JSON.stringify({
      type: "session",
      version: 3,
    ...
    JSON.stringify(userEntry("cu1", "si1", "Task: do the delegated work")),
  ];
  if (lastRole === "assistant") {
    lines.push(JSON.stringify(assistantEntry("ca1", "cu1", "final answer", "2026-01-01T00:00:03.000Z")));
  } else {
    lines.push(JSON.stringify({
      ...
      parentId: "cu1",
      timestamp: "2026-01-01T00:00:03.000Z",
      message: { role: "toolResult", toolCallId: "t1", content: [{ type: "text", text: "mid-run" }] },
    }));
  }
...
test("derives the parent id from the nested subagent layout only", () => {
  const sessionsDir = join(tmpdir(), "pi-web-nested-layout", "sessions");
  const projectDir = join(sessionsDir, "--project--");
  const parentStem = "2026-01-01T00-00-00-000Z_parent-uuid";

  assert.equal(
    deriveNestedSubagentParentId(
      join(projectDir, parentStem, "child-uuid", "run-0", "session.jsonl"),
      sessionsDir,
    ),
    "parent-uuid",
  );
  // Top-level flat sessions are not nested subagents.
  assert.equal(
    deriveNestedSubagentParentId(
      join(projectDir, "2026-01-01T00-00-00-000Z_child.jsonl"),
      sessionsDir,
    ),
    null,
  );
  // Unrelated nested directories without a parent-stem name are skipped.
  assert.equal(
    deriveNestedSubagentParentId(
      join(projectDir, "random-dir", "child.jsonl"),
      sessionsDir,
    ),
    null,
  );
...
test("infers subagent status from the final message role", () => {
  assert.equal(inferSubagentStatus("assistant"), "completed");
  assert.equal(inferSubagentStatus("toolResult"), "interrupted");
  assert.equal(inferSubagentStatus(undefined), "interrupted");
});

test("truncates long subagent descriptions without changing short ones", () => {
  assert.equal(truncateSubagentDescription("  hello   world "), "hello world");
  const long = truncateSubagentDescription("a".repeat(200));
  assert.equal(long.length, 120);
  assert.ok(long.endsWith("…"));
});

test("discovers nested subagent sessions with parent relation and status", async (t) => {
  const agentDir = mkdtempSync(join(tmpdir(), "pi-web-nested-discovery-"));
  const parentId = "parent-uuid-1";
  const childId = "child-uuid-1";
  const interruptedId = "child-uuid-2";
  const { childPath, parentPath } = writeNestedSubagentLayout(agentDir, {
    parentId,
    childId,
    name: "subagent-delegate-call-1",
    lastRole: "assistant",
  });
  const { childPath: interruptedPath } = writeNestedSubagentLayout(
    agentDir,
    { parentId, childId: interruptedId, name: undefined, lastRole: "toolResult" },
  );

  setTestAgentDir(t, agentDir);

  const nested = await listNestedSubagentSessions();
  assert.deepEqual(nested.map((session) => session.id).sort(), [childId, interruptedId].sort());

  const child = nested.find((session) => session.id === childId);
  assert.equal(child.path, childPath);
  assert.equal(child.cwd, "/tmp/project");
  assert.equal(child.name, "subagent-delegate-call-1");
  assert.equal(child.messageCount, 2);
  assert.equal(child.firstMessage, "Task: do the delegated work");
  assert.equal(child.modified, "2026-01-01T00:00:03.000Z");
  assert.deepEqual(child.relation, {
    kind: "subagent",
    parentSessionId: parentId,
    profile: "pi-subagent",
    description: "Task: do the delegated work",
    status: "completed",
  });

  const interrupted = nested.find((session) => session.id === interruptedId);
  assert.equal(interrupted.relation.status, "interrupted");
  assert.equal(interrupted.relation.description, "Task: do the delegated work");

  // The merged listing exposes the nested session and resolves its path.
  resetSessionPathState();
  const all = await listAllSessions({ force: true });
  assert.ok(all.some((session) => session.id === childId));
  assert.ok(all.some((session) => session.id === parentId));
  assert.equal(await resolveSessionPath(childId), childPath);
  assert.equal(await resolveSessionPath(interruptedId), interruptedPath);

  // The same file is not duplicated by the flat SDK scan.
  const originalListAll = SessionManager.listAll;
  SessionManager.listAll = async () => [{
    path: childPath,
    id: childId,
    cwd: "/tmp/project",
    created: new Date("2026-01-01T00:00:01.000Z"),
    modified: new Date("2026-01-01T00:00:03.000Z"),
    messageCount: 2,
    firstMessage: "Task: do the delegated work",
    allMessagesText: "",
    parentSessionPath: parentPath,
  }];
  t.after(() => {
  ...
  resetSessionListState();
  const deduped = await listAllSessions({ force: true });
  assert.deepEqual(deduped.filter((session) => session.id === childId).length, 1);
});

test("ignores malformed nested files and non-session directories", async (t) => {
  const agentDir = mkdtempSync(join(tmpdir(), "pi-web-nested-invalid-"));
  const projectDir = join(agentDir, "sessions", "--project--");
  const parentStem = "2026-01-01T00-00-00-000Z_parent-uuid";
  mkdirSync(join(projectDir, parentStem, "child-1", "run-0"), { recursive: true });
  writeFileSync(join(projectDir, parentStem, "child-1", "run-0", "session.jsonl"), "{not-json\n");
  // A directory named like a parent stem but containing no valid sessions.
  mkdirSync(join(projectDir, parentStem, "child-2", "run-0"), { recursive: true });
  writeFileSync(
    join(projectDir, parentStem, "child-2", "run-0", "session.jsonl"),
    `${JSON.stringify({ type: "not-session", id: "x" })}\n`,
  );
  // A non-stem directory is skipped entirely.
  mkdirSync(join(projectDir, "other", "child-3"), { recursive: true });
  writeFileSync(
    join(projectDir, "other", "child-3", "session.jsonl"),
    `${JSON.stringify({ type: "session", version: 3, id: "child-3", timestamp: "2026-01-01T00:00:00.000Z", cwd: "/tmp/project" })}\n`,
  );

  setTestAgentDir(t, agentDir);

  assert.deepEqual(await listNestedSubagentSessions(), []);
});
