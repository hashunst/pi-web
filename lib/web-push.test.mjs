...
const SUB_FA = {
  endpoint: "https://push.example.com/fa",
  keys: { p256dh: "p256dh-fa", auth: "auth-fa" },
  locale: "fa",
};
...
test("notifySessionComplete sends Persian payloads for fa subscribers", async () => {
  const { notifier, sent } = makeEnvironment({ sessionNames: new Map() });
  notifier.addSubscription(SUB_FA);

  await notifier.notifySessionComplete("session-1");

  const fa = sent.find((s) => s.subscription.endpoint === SUB_FA.endpoint);
  assert.deepEqual(JSON.parse(fa.payload), {
    title: "جلسه تمام شد",
    body: "کار تمام شد.",
    url: "/?session=session-1",
    tag: "pi-session-complete:session-1",
...
test("localeText falls back to English for unknown locales", () => {
  ...
  assert.equal(localeText("fa", "taskFinished"), "کار تمام شد.");
  assert.equal(localeText("fa", "sessionComplete"), "جلسه تمام شد");
