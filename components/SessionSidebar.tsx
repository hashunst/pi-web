...
export function SessionSidebar({ selectedSessionId, onSelectSession, onNewSession, initialSessionId, skipInitialProjectSelection, onInitialRestoreDone, refreshKey, onSessionDeleted, selectedCwd: selectedCwdProp, onCwdChange, onOpenFile, onOpenTerminal, explorerRefreshKey, onExplorerRefresh, onAtMention, onAtMentions, onBackgroundTaskDone, onRunningSessionIdsChange, onSessionsChange }: Props) {
  ...
  // Flatten families into fixed-height rows: the root row plus, unless the
  // family is collapsed, one row per subagent. Keeps the list windowed while
  // rendering nested subagent sessions.
  const sessionListRows: { family: SessionFamily; subagent?: SessionInfo }[] = [];
  for (const family of sessionFamilies) {
    sessionListRows.push({ family });
    if (!collapsedSubagentFamilies.has(family.root.id)) {
      for (const subagent of [...family.subagents].sort((a, b) => b.modified.localeCompare(a.modified))) {
        sessionListRows.push({ family, subagent });
      }
    }
  }
  ...
  const virtualIndices = getSessionListIndices(
    ...
    listScrollTop,
    listViewportH,
    sessionListRows.findIndex((row) => (row.subagent ?? row.family.root).id === focusedSessionId),
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
      ...
      >
        ...
        {sessionListRows.length > 0 && (
          <div
            style={{
              position: "relative",
              height: sessionListRows.length * SESSION_LIST_ITEM_HEIGHT,
            }}
          >
            {virtualIndices.map((index) => {
              ...
              if (subagent) {
                return (
                  <div
                    key={subagent.id}
                    onFocus={() => setFocusedSessionId(subagent.id)}
              ...
              return (
                ...
                >
                  <SessionItem
                    ...
                    collapsed={collapsedSubagentFamilies.has(family.root.id)}
